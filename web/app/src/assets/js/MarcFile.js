...
function MarcFile(source, onLoad){	
	...
	if(typeof source==='object' && source.name && source.size){ /* source is file */
		...
		this._fileReader.addEventListener('load',function(readerEvent){
			this._u8array=new Uint8Array(readerEvent.target.result);
			this._dataView=new DataView(readerEvent.target.result);
